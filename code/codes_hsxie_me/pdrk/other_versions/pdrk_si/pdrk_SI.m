...
B0=100.0E-9; c2=(2.9979E8)^2; epsilon0=8.854E-12 ; 
mu0=1/(c2*epsilon0);
kB=1.38e-23;
...
for s=1:S
    ...
    Tzs0(s)=par.data(s,4); % parallel temperature
    Tps0(s)=par.data(s,5); % perp temperature
    vs0(s)=par.data(s,6); % para drift velocity
end

qs=qs0*1.602e-19;
ms=ms0*9.109E-31;
Tzs=Tzs0*1.6e-19/kB; % eV -> K
Tps=Tps0*1.6e-19/kB;

vtzs=sqrt(2*kB*Tzs./ms); % para thermal velocity
...
% wg=(0.6-0.01i)*wci;
% wg=(0.3-0.01i)*wci;
for kj=1:-0.01:0.01%kj=0.01:0.01:1.0% 
%     kz=kj/5; kx=kz/2;
%     kz=kj/100; kx=kj/1;
    theta=0*pi/180;
...
if(J==8)
    subplot(121);
    plot(kplot,real(wplot),'b','LineWidth',2); hold on;
%     plot(kk*cwp,real(ww(1,:)/wci),'bx','LineWidth',2); hold on;
    set(gca,'xscale','log');
    ylim([0.1,0.6]); xlim([0.01,1.0]);
    xlabel('kc/\omega_{pi}'); ylabel('\omega_r/\Omega_{ci}');
    subplot(122);
    plot(kplot,imag(wplot),'b','LineWidth',2); hold on;
%     semilogx(kk*cwp,imag(ww(1,:)/wci),'bx','LineWidth',2); hold on;
%     ylim([-0.02,0.05]);
    ylim([-0.1,0.1]); xlim([0.01,1.0]);
    set(gca,'xscale','log');
    xlabel('kc/\omega_{pi}'); ylabel('\omega_r/\Omega_{ci}');
else
    subplot(121);
