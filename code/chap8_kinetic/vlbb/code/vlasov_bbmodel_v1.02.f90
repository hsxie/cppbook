...
!*              vlasov_bbmodel_v1.02.f90, 2011-08-26 07:15                         *
!*        Hua-sheng XIE, IFTS-ZJU, huashengxie@{gmail.com, zju.edu.cn}             *
!***********************************************************************************

module variable
	implicit real*8 (A-H,O-Z)
	integer :: stdout=0, method_efield=2, method_vlasov=3
	...
	!                0. Fij99; 1. collisonless; 2. Krook, Vann03; 3. Krook + drag
	!                + diffusion, Lesur10.

	real*8 :: gammad=1.0d0, mua=0.03d0, muf=0.d0, mud=0.d0
	! gammad: damping rate for BB model
	! mua: Krook collison for BB model
	! muf: friction term (drag) in BB model
	! mud: diffusion term in BB model
	integer,parameter :: m=63, n=64, nt=30000, nplot=1000, ndiag=1
	...
	real*8,parameter :: rk0=.2d0, vmax=12.0d0, dt=0.05d0, pi=3.14159265358979323846
	...
	real*8,parameter :: dv=vmax/dfloat(m), xl=2.d0*pi/rk0, dx=xl/dfloat(n)
	...
	real*8,parameter :: alpha=0.1d0, fbeam=1.0
	! alpha: perturbation parameter
	! fbeam=1.0: two-stream/beam distribution; else: Maxwellian distribution

	real*8,parameter :: vtc=1.0d0, vtb=0.5d0, vdc=-0.0d0, vdb=4.5d0, nb=0.1d0
...
subroutine vlasov
	...
	select case(method_vlasov)
		...
		case(1)                                                ! Collisonless Vlasov
			call shiftx1
			call efield
			call shiftv
			call shiftx1

		case(2)                                         ! Krook for BB model, Vann03
			call shiftx
			call krook
			call efield2
			call shiftv
		...
		case(3)                     ! Krook + drag + diffusion for BB model, Lesur10
			call shiftx
			call diffusion
			call efield2
			call shiftv
	...
	do i=0,n-1
		...
		do j=-m,m
			if(fbeam==1) then
				...
				!      f0c(v)=(1-nb)/(vtc*sqrt(2*pi))*exp(-((v-vdc)/(2*vtc))^2)
				!      f0b(v)=nb/(vtb*sqrt(2*pi))*exp(-((v-vdb)/(2*vtb))^2)
				F0(i,j)=(cte*(1-nb)/vtc*dexp(-0.5d0*((v(j)-vdc)/(vtc))**2)+&
				 cte*nb/vtb*dexp(-0.5d0*((v(j)-vdb)/(vtb))**2))
			else
				! f(i,j): Maxwellian in V.
				F0(i,j)=cte*dexp(-v(j)*v(j)/2.0d0)
			endif
			f(i,j)=F0(i,j)*(1.d0+pert)
		enddo
	enddo
	ef(-1:n)=alpha*dcos(rk0*x(-1:n))                                ! initial Efield
...
!*---------------------------------------------------------------------------------*
subroutine vlasov2                                   !  Routine from Watt 'mccorm_e'

	use variable
	implicit real*8 (A-H,O-Z)

	real*8,dimension(-1:n) :: jx,exminus1

	

end subroutine vlasov2
...
subroutine shiftv                     ! Subroutine 'SHIFTV' of Fijalkow1999([1]&[2])
	...
	real*8,dimension(-1:n) ::  efn
	! y: Temporary storage for the distribution function to be shifted
	! df: Phase space "mass" shifted from cell j to j+1
	! efn: ef new, efn(x)=ef(x)-muf^2/rk0, to solve df/dt+efn*df/dv=0

	efn(-1:n)=ef(-1:n)-muf**2/rk0

	do i=0,n-1                        ! loop on all the grid point in the variable X
		if(efn(i) > 0.d0) then        ! shift for electric field positive values  
		                                ! previously calculated by a call to efield.
			shh=efn(i)*dtodv                        ! value of the shift, normalized
			...
			shh=-efn(i)*dtodv                              ! shh: value of the shift
...
!	if(t==0.d0) call efield1                                ! Get the initial Efield
	...
	do i=0,n-1
		hh=0.d0
		do j=-m,m
		!	hh=hh-v(j)*(f(i,j)-f_0(j))
			hh=hh-v(j)*(f(i,j)-favg)
		!	hh=hh-v(j)*f(i,j)
		enddo
		! def(i)=hh*dv
		def(i)=hh*dv
...
!*---------------------------------------------------------------------------------*
subroutine krook                                       ! Krook collison for BB model
	...
	do i=0,n-1
		do j=-m,m
			f(i,j)=f(i,j)-mua*(f(i,j)-F0(i,j))*0.5d0*dt
		enddo
	enddo

end subroutine krook

!*---------------------------------------------------------------------------------*
subroutine diffusion                         ! Krook + drag + diffusion for BB model

	use variable
	implicit real*8 (A-H,O-Z)

	real*8,dimension(-1:n,-m:m) :: dvF0, dvvF0, dvvf

	do i=-1,n
		do j=-m+1,m-1
			dvF0(i,j)=0.5d0*(F0(i,j+1)-F0(i,j-1))/dv
			dvvF0(i,j)=(F0(i,j+1)-2.d0*F0(i,j)+F0(i,j-1))/dv**2
			dvvf(i,j)=(f(i,j+1)-2.d0*f(i,j)+f(i,j-1))/dv**2
		enddo
		dvF0(i,-m)=0.d0
		dvF0(i,m)=0.d0
		dvvF0(i,-m)=0.d0
	...
	do i=0,n-1
		do j=-m,m
			f(i,j)=f(i,j)-mua*(f(i,j)-F0(i,j))*0.5d0*dt-muf**2/rk0*dvF0(i,j)*dt+&
			       mud**3/rk0**2*(dvvf(i,j)-dvvF0(i,j))*dt
		enddo
	enddo
...
subroutine history                                             ! Diagnostics routine
	...
	!           mean density: <N>
	!           mean momentum: <P>
	!           mean energies: 1. kinetic <Ekin>; 2. electric <Eelect>; 
	!                          3. total <Etotal>=<Ekin>+<Eelect>.
	!                                                                 L
	!                    where <g>=1/L*{sum over X of a function g(X)}
	!                                                                 0
	!           max eletric energy: max(|E(x)|)
	use variable
	implicit real*8 (A-H,O-Z)
	...
	efmax=dabs(maxval(ef(-1:n)))

	if(it==1) et0=eek+eef                             ! Get the initial total energy

	! Output to an external file
	!   t: time; fn: mean density; efmax: max eletric energy; pp: mean momentum; eek:
	!   kinetic energy; eef: mean electric energy; eek+eef: total energy; s1,s2:
	!   entropy
	write(15,'(1x,F9.3,8ES18.8)') t, fn, pp, efmax, eek, eef, eek+eef, s1, s2
...
!*---------------------------------------------------------------------------------*
!subroutine fcheck                       ! A routine check f(v) at v=vmax and v=-vmax
!
!	use variable
!	implicit real*8 (A-H,O-Z)
!	real*8 :: fb
!
!	fb=maxval(sum(f(0:n-1,m))/dfloat(n))
!
!
